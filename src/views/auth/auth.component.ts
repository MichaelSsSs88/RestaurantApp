...
import { Observable, Subscriber, Subscription, interval, timeout } from 'rxjs';
...
export class AuthComponent implements AfterContentChecked, OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(private cd: ChangeDetectorRef, private authService: AuthService, private authentication: AuthenticationService, private router: Router, private store:Store<{counter:number}>) {
  ...
  nameValitator(control: AbstractControl): string[] {
    ...
    if (control.value == '' && control.touched) {
      error.push($localize`${"Please type the name"}`);
    }
    if (control.value != '' && control.value.length < 10) {
      error.push($localize`${"Name has to be at least 10 characters long."}`);
    }
    return error;
  }
  ...
  displayErros(controlName: string): string[] {
    ...
    value.touched && value.errors && controlName === "email" && errors.push($localize`${"It must be a valid email address"}`)
    value.touched && value.errors && controlName === "password" && errors.push($localize`${"It must be a valid password"}`)
    ...
    value.touched && controlName === "confirmPassword" && (this.formData.value.password != this.formData.value.confirmPassword) && errors.push($localize`${"The password confirmation must match the password"}`)
    return errors;
  }
    ...
    this.authentication
      ...
      .subscribe({
        next: (data) => {
          ...
          this.showErrorAlert($localize`${"The username and the password does not match"}`);
