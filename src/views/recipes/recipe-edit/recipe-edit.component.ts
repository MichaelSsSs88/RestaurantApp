...
export class RecipeEditComponent implements OnInit, OnDestroy{
  ...
  constructor(private cd: ChangeDetectorRef, private router: Router,private route:ActivatedRoute,private recipeService: RecipeService, private recipeEntityService: RecipeEntityService){
    ...
    this.subscriber = this.router.events.pipe(
     ...
     //this.index?this.recipeEntityService.entities$.subscribe(recipe=>{recipe.forEach((recipe)=>{if(recipe.id==this.index)this.recipeSelected=recipe})}):this.recipeSelected= new recipe("","","","",new Array<ingredient>());
    });
  ...
  ngOnInit(): void {
    this.index?this.recipeEntityService.entities$.subscribe(recipe=>{recipe.forEach((recipe)=>{if(recipe.id==this.index)this.recipeSelected=<recipe>recipe})}):this.recipeSelected= new recipe("","","","",new Array<ingredient>());
    //this.index?this.recipeSelected = this.recipeService.getRecipeById(this.index):this.recipeSelected= new recipe("","","","",new Array<ingredient>());
  ...
  onSubmit():void{
    this.recipeSelected= new recipe(this.index,this.recipeForm.value.name,this.recipeForm.value.description,this.recipeForm.value.imagePath,this.recipeSelected._ingredients)
    // this.recipeSelected._name=this.recipeForm.value.name;
    // this.recipeSelected._description=this.recipeForm.value.description;
    // this.recipeSelected._imagePath=this.recipeForm.value.imagePath;
    // this.recipeSelected._ingredients=this.recipeForm.value.ingredients
    //this.index?this.recipeService.updateRecipe(parseInt(this.index),this.recipeSelected):this.recipeService.addRecipe(this.recipeSelected);
    this.index?this.recipeEntityService.update(this.recipeSelected):this.recipeService.addRecipe(this.recipeSelected);
    this.recipeForm.reset();
    this.ingredientId=null;
    this.ingredient=null;
    this.recipeSelected= new recipe("","","","",new Array<ingredient>());
    this.index&&this.router.navigateByUrl('/home/detail');
  ...
  showError(controlName:string):any[]{
    ...
    controlName=="description"&&(this.class_description = "peer-focus:font-medium absolute text-sm text-gray-500 dark:text-white duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-white peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6",
                          this.class_description_input="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-2 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-white dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer mt-2"
    );
    return error;
  }
  ...
  public getError(controlName: string): any[] {
    ...
    controlName=="name_ingredient"&&(this.class_ingredient = "peer-focus:font-medium absolute text-sm text-gray-500 dark:text-white dark:border-white duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-white peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6");
