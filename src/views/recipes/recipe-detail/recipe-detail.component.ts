...
export class RecipeDetailComponent implements OnInit,OnChanges,OnDestroy {
  ...
  public recipe: Subscription;


  constructor(public recipeService: RecipeService, private shoppingService:ShoppingService, private _router: Router, private route:ActivatedRoute, private recipeEntityService:RecipeEntityService){
    ...
    //   this.recipe=this.recipeEntityService.entities$.subscribe((recipe)=>{
    //     recipe.forEach((recipe)=>{if(recipe.id==this.index){this.recipeSelected=recipe}})
    // });
    //  // this.recipeEntityService.getByKey(this.index).subscribe(recipe =>{
